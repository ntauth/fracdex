...
func TestAddJitterToKey(t *testing.T) {
	t.Run("Basic jitter", func(t *testing.T) {
		originalKey := "a1V" // Has fractional part
		r := rand.New(rand.NewSource(42))
		jitter := RandJitter{R: r}

		// Jitter the key
		jitteredKey, err := JitterKey(originalKey, jitter, 2)
		if err != nil {
			t.Fatalf("JitterKey failed: %v", err)
		}

		// The jittered key should have the SAME length as the original
		if len(jitteredKey) != len(originalKey) {
			t.Errorf("Expected same length, got %s (length: %d) vs %s (length: %d)",
				jitteredKey, len(jitteredKey), originalKey, len(originalKey))
		}

		// The jittered key should still be a valid order key
		if err := validateOrderKey(jitteredKey); err != nil {
			t.Errorf("Jittered key %s is not valid: %v", jitteredKey, err)
		}

		// The jittered key should not end with '0'
		if strings.HasSuffix(jitteredKey, "0") {
			t.Errorf("Jittered key %s ends with '0', violating invariant", jitteredKey)
		}

		t.Logf("Original key: %s, Jittered key: %s", originalKey, jitteredKey)
	})

	t.Run("Multiple iterations produce variation", func(t *testing.T) {
		originalKey := "a1V"
		results := make(map[string]bool)
		...
		for i := range 50 {
			...
			jitteredKey, err := JitterKey(originalKey, jitter, 2)
			if err != nil {
				t.Fatalf("JitterKey failed on iteration %d: %v", i, err)
			}

			results[jitteredKey] = true
		}

		// Should get multiple different results
		if len(results) <= 1 {
			t.Errorf("Expected multiple jittered keys, got only %d: %v", len(results), results)
		} else {
			t.Logf("Generated %d unique jittered keys, demonstrating variation", len(results))
		}
	})

	t.Run("NoJitter produces no change", func(t *testing.T) {
		originalKey := "a1V"
		noJitter := NoJitter{}

		jitteredKey, err := JitterKey(originalKey, noJitter, 3)
		if err != nil {
			t.Fatalf("JitterKey failed: %v", err)
		}

		// NoJitter should return the original key unchanged
		if jitteredKey != originalKey {
			t.Errorf("Expected no change with NoJitter, got %s", jitteredKey)
		}
	})

	t.Run("Zero jitter range produces no change", func(t *testing.T) {
		originalKey := "a1V"
		r := rand.New(rand.NewSource(42))
		jitter := RandJitter{R: r}

		jitteredKey, err := JitterKey(originalKey, jitter, 0)
		if err != nil {
			t.Fatalf("JitterKey failed: %v", err)
		}

		// Zero jitter range should return the original key unchanged
		if jitteredKey != originalKey {
			t.Errorf("Expected no change with zero jitter range, got %s", jitteredKey)
		}
	})

	t.Run("Keys without fractional parts", func(t *testing.T) {
		// Test with keys that have no fractional part (like "a1")
		originalKey := "a1"
		r := rand.New(rand.NewSource(42))
		jitter := RandJitter{R: r}
		...
		// For keys without fractional parts, jitter might not be possible
		// so we expect either the same key or a valid alternative
		if jitteredKey != originalKey {
			// If we got a different key, verify it's valid
			if err := validateOrderKey(jitteredKey); err != nil {
				t.Errorf("Jittered key %s is not valid: %v", jitteredKey, err)
			}
			t.Logf("Key without fractional part was jittered: %s -> %s", originalKey, jitteredKey)
		} else {
			t.Logf("Key without fractional part could not be jittered (expected)")
		}
	})
}
